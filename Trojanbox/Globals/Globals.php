...
use Trojanbox\Globals\Exception\GlobalsException;
...
class Globals {
	...
	public function set($name, $objcet, $mvc = 'MVC') {
		if (empty($name) || empty($mvc)) throw new GlobalsException(__METHOD__ . '不接受空参数！');
		$mvc = strtoupper($mvc);
		$mvcLen = strlen($mvc);
		for ($i = 0; $i < $mvcLen; $i++) {
			switch ($mvc{$i}) {
				...
				case 'C':
					...
					break;
				default:
					throw new GlobalsException(__METHOD__ . '不接受除MVC外的未知参数！');
					break;
			}
		}
