...
class TerminalManager implements TerminalManagerInterface {
	...
	public function cmd($string = null, $mode = self::SCP) {
		if (empty($string)) throw new TerminalException('Terminal 适配器需要一个String参数！');
	...
	public function key($key = null) {
		if (empty($key)) throw new TerminalException('Terminal Key 需要一个参数开关！');
		if (!$this->tRunState) $this->coreResolver($this->tString);
    ...
    private function linuxResolver($key) {
    	if (empty($this->tArgsStruct)) return false;
