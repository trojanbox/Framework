...
use Trojanbox\Framework\Exception\ModelException;
...
class Model {
	 ...
	 * @return \Trojanbox\Framework\Model
	 */
	private function _execTableName() {
		...
		if (empty($tableName)) {
			throw new ModelException('模型：获取表名失败！<br>可能原因：解析命名空间过程中出错！');
		}
		...
		if (empty($this->_TableName)) {
			throw new ModelException('模型：在创建表名过程中出现严重问题！');
		}
