...
class Route {
	...
	public function setUrl($url = null) {
		if (empty($url)) throw new \Exception('这不是一个URL');
		$this->url = $url;
		$this->_execRoute();
		return $this;
	}
	...
	public function getUrl() {
		if (empty($this->url)) throw new \Exception('请执行' . __CLASS__ . '::setUrl()进行赋值操作！');
		return $this->MUrl;
	}
	...
	final protected function _execRoute() {
		...
		foreach ($this->config as $key => $value) if (preg_match($key, $this->url, $returnUrlParams)) {
			preg_match_all('/\$(\d+)/', $value, $trueParams);
			foreach ($trueParams[1] as $key2 => $value2) $url[] = $returnUrlParams[$value2];
